...
object dryrunfuad {
  def main(args: Array[String]): Unit = {
    ...
    val df_ethereum = spark.read.jdbc(url, "ethereum7", properties)
    df_ethereum.show(false)
    // Create Hive Internal table
    df_ethereum.write.mode(SaveMode.Overwrite).saveAsTable("scalagroup.Ethereum_InitialDataFrame7")
    ...
    // Create Hive Internal table
    filtered_df_ethereum.write.mode(SaveMode.Overwrite).saveAsTable("scalagroup.Ethereum_FilteredByPrice7")
    ...
    // Create Hive Internal table
    sorted_df_ethereum.write.mode(SaveMode.Overwrite).saveAsTable("scalagroup.ethereum_sortedbykeybyprice7")
    ...
    mean_price_df_ethereum.write.mode(SaveMode.Overwrite).saveAsTable("scalagroup.ethereum_mean_price7")
    ...
    // Create Hive Internal table
    filtered_df_bitcoin.write.mode(SaveMode.Overwrite).saveAsTable("scalagroup.bitcoin_filteredbyprice7")
    ...
    mean_price_df_bitcoin.write.mode(SaveMode.Overwrite).saveAsTable("scalagroup.bitcoin_mean_price7")
    ...
    df_join.show(false)
    df_join.write.mode(SaveMode.Overwrite).saveAsTable("scalagroup.bitcoinjoinethereum7")
