...
object incrementalFuad {
  def main(args: Array[String]): Unit = {
    ...
    df_ethereum.write.mode(SaveMode.Append).saveAsTable("scalagroup.ethereum_initialdataframe7")

    // *****************************************************************************************************

    val max_filtered_df_ethereum = spark.sql("select max(ethereum_id) from scalagroup.ethereum_filteredbyprice7").first()
    ...
    filtered_df_ethereum.write.mode(SaveMode.Append).saveAsTable("scalagroup.ethereum_filteredbyprice7")

    // *****************************************************************************************************
    val max_sorted_df_ethereum = spark.sql("select max(ethereum_id) from scalagroup.ethereum_sortedbykeybyprice7").first()
    ...
    sorted_df_ethereum.write.mode(SaveMode.Append).saveAsTable("scalagroup.ethereum_sortedbykeybyprice7")

    // *****************************************************************************************************
    val max_mean_price_df_ethereum = spark.sql("select max(ethereum_id) from scalagroup.ethereum_mean_price7").first()
    ...
    mean_price_df_ethereum_no_null.write.mode(SaveMode.Append).saveAsTable("scalagroup.ethereum_mean_price7")

    // *****************************************************************************************************
    // Bitcoin Incremental
    // *****************************************************************************************************

    val max_df_bitcoin = spark.sql("select max(bitcoin_id) from scalagroup.bitcoin_initialdataframe7").first()
    ...
    df_bitcoin.write.mode(SaveMode.Append).saveAsTable("scalagroup.bitcoin_initialdataframe7")

    // *****************************************************************************************************

    val max_filtered_df_bitcoin = spark.sql("select max(bitcoin_id) from scalagroup.bitcoin_filteredbyprice7").first()
    ...
    filtered_df_bitcoin.write.mode(SaveMode.Append).saveAsTable("scalagroup.bitcoin_filteredbyprice7")

    // *****************************************************************************************************
    val max_sorted_df_bitcoin = spark.sql("select max(bitcoin_id) from scalagroup.bitcoin_sortedbykeybyprice7").first()
    ...
    sorted_df_bitcoin.write.mode(SaveMode.Append).saveAsTable("scalagroup.bitcoin_sortedbykeybyprice7")

    // *****************************************************************************************************
    val max_mean_price_df_bitcoin = spark.sql("select max(bitcoin_id) from scalagroup.bitcoin_mean_price7").first()
    ...
    mean_price_df_bitcoin_no_null.write.mode(SaveMode.Append).saveAsTable("scalagroup.bitcoin_mean_price7")

    // *****************************************************************************************************
    // Ethereum Filtered Join Bitcoin Filtered Incremental
    // *****************************************************************************************************
    val max_df_ethereum_join_df_bitcoin = spark.sql("select max(bitcoin_id) from scalagroup.bitcoinjoinethereum7").first()
    ...
    df_ethereum_join_df_bitcoin.write.mode(SaveMode.Append).saveAsTable("scalagroup.bitcoinjoinethereum7")
  }
}
